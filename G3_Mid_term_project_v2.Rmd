...
The amount of data we are working with and the frequency of observations allows us to avoid statistical mistakes and obtain a robust regression analysis that could provide relevant insight on unemployment. Having seen how sex and education change through the sample through state and the different percentage of unemployed receiving the benefit, using sex, education, and age as control variables will allow us in the next steps to obtain relevant conclusions that can contribute in informing employment policy in US.Under the exploration of the current dataset we were able to clean raw data and be able to start understanding some preliminary trends between the surveyed population. However, additional analysis it is still required given that correlation and causality are not necessarily the same, however the dataset provides the opportunity that is able to unmask the impacts that the unemployment insurance had before and during COVID. Moreover, given that the survey its still rolling it could also provide more relevant indicators that could should if the prediction we made in our modelling are actually materializing. Although the EDA did provide us a better understanding of our data and the variables that we can potentially use as control variables in our regression analysis, we are not able to answer out SMART question at this point given that we would not be able to justify the statistical robustness of our findings. 
