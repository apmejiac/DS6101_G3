...
#renaming columns
#colnames(df_panel)[1:20]=c("ID","Week","State","Age","Hispanic","Other_race","Edu","Sex","p_hh","wrklossrv", 'anywork','kindwork','rsnnowrkrv','UI_apply', 'UI_recrv','UI_recvnow','house', 'income','SPND1, )
head(df_panel)

#as_date(df_panel$Age, origin = lubridate::origin)
#age_calc(df_panel$Age, enddate = Sys.Date(), units = "years", precise = TRUE)
#df_panel$TBIRTH_YEAR = as.numeric(difftime(Sys.Date(),df_panel$TBIRTH_YEAR))

#str(df_panel)
#df_panel$new_Age <- strptime(x, format = "%Y%m%d") 
#df_panel$x_date2 <-  
#summary(df_panel)
#skim(df_panel)
#panel.hist(df_panel$RHISPANIC, col.bar = "gray")
#panel.histogram(,
#                breaks =20,
#                equal.widths = TRUE,
#                type = "density",
